...
CONFIG_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "dashboard-config.json"
)
...
def get_session_stats(session_path: str) -> Optional[Dict[str, Any]]:
    ...
    end_time_ms = last_msg.get("time", {}).get(
        "completed", last_msg.get("time", {}).get("created", start_time_ms)
    )
    ...
    for m_name, data in models_used.items():
        models_list.append(
            {"name": m_name, "tokens": data["tokens"], "cost": f"${data['cost']:.2f}"}
        )

    recent_tokens = {"input": 0, "output": 0, "cache_write": 0, "cache_read": 0}
    ...
    for m in reversed(messages):
        if m.get("tokens") is not None and isinstance(m["tokens"], dict):
            ...
            if msg_input > 0 or msg_cache_read > 0:
                ...
                recent_tokens["cache_write"] = (
                    m["tokens"].get("cache", {}).get("write", 0) or 0
                )
    ...
    if context_window > 0:
        ...
        context_percentage = min(
            100, int((current_turn_context / context_window) * 100)
        )
    ...
    if last_msg.get("time", {}).get("completed") and last_msg.get("time", {}).get(
        "created"
    ):
        last_msg_duration_ms = (
            last_msg["time"]["completed"] - last_msg["time"]["created"]
        )
    ...
    return {
        "id": session_id,
        "started": datetime.fromtimestamp(start_time_ms / 1000).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
        ...
        "last_activity": datetime.fromtimestamp(last_activity_ms / 1000).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
            ...
            for d in session_dirs:
                ...
                if os.path.isdir(path):
                    ...
                    if isinstance(stats, dict):
                        ...
                        merge_stats(
                            overall_agent_stats, stats_dict.get("agent_stats", {})
                        )
                        merge_stats(
                            overall_model_stats, stats_dict.get("model_stats", {})
                        )
                        ...
                        if is_recent(stats_dict.get("timestamp"), today_start_ts):
                            ...
                            stats_tokens_val: int = to_int(
                                stats_dict.get("total_tokens")
                            )
